...
fn build_vendored(link_mode: LinkMode) {
    ...
    if let Some(font_backend) = zig_font_backend(&target) {
        build.arg(format!("-Dfont-backend={font_backend}"));
    }
    ...
    if target != host {
        ...
        if let Some(cpu) = zig_cpu(&target) {
...
fn zig_cpu(target: &str) -> Option<&'static str> {
    match target {
        // Ghostty's own XCFramework path uses apple_a17 for the arm64 iOS
        // simulator slice; Zig 0.15 otherwise compiles simdutf without the
        // target features required by the SDK's NEON intrinsics.
        "aarch64-apple-ios-sim" => Some("apple_a17"),
        _ => None,
    }
}

fn zig_font_backend(target: &str) -> Option<&'static str> {
    match target {
        // Shellow links libghostty-vt for terminal state only; font discovery,
        // shaping, and rasterization live in Shellow's renderer. Android's
        // default Ghostty backend pulls fontconfig/harfbuzz/libxml2, which are
        // unrelated to lib-vt and make mobile vendored builds much heavier.
        "aarch64-linux-android" | "x86_64-linux-android" => Some("coretext_noshape"),
        _ => None,
    }
}
